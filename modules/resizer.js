...
function calculateNewSize(query, callback){
  ...
  easyimg.info(getImagePath(query.source), function(err, stdout, stderr){
    ...
    // imageSize.fill = true
    // imageSize.cropwidth = imageSize.width
    // imageSize.cropheight = imageSize.height
    // console.log('dux');
...
function getImagePath(source){
  var savePath = ORGINAL_IMAGE_PATH + getImageName(source);
